...
test_coverage:
	$(BIN)pytest \
		-v \
		--cov=submitit_oar --cov-report=html --cov-report=term \
		--durations=10 \
...
check_format:
	# also formats docs
	$(BIN)isort --check --diff $(CODE)
	$(BIN)black --check --diff $(CODE)
...
installable_local: venv
	(. ./venv/bin/activate ; cd /tmp ; python -c "import submitit_oar")

USER_VENV=/tmp/submitit_oar_user_venv/
CURRENT_VERSION=`grep -e '__version__' ./submitit_oar/__init__.py | sed 's/__version__ = //' | sed 's/"//g'`
